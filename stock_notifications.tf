...
resource "aws_iam_policy" "ssm_policy" {
  ...
  description = "Allow Lambda to read ALPHA VANTAGE API Key from Parameter Store"
  ...
  "Statement": [
    {
      ...
      "Resource": "arn:aws:ssm:us-east-1:${data.aws_caller_identity.current.account_id}:parameter/stock-api-key"
    }
  ]
}
EOF
...
resource "aws_lambda_function" "stock_lambda" {
  filename      = "stock_notifications.zip" # Pre-packaged ZIP
  function_name = "stock_alerts"
  role          = aws_iam_role.lambda_role.arn
  handler       = "stock_notifications.lambda_handler"
  runtime       = "python3.8"
